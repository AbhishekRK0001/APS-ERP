...
export default function SubstituteRequests() {
  ...
  useEffect(() => {
    API.get('/substitutes/my').then(r => setRequests(r.data)).catch(() => {});
  }, []);

  const accept = async (id) => {
    try {
      await API.patch(`/substitutes/${id}/accept`);
      toast.success('Coverage accepted successfully.');
      setRequests(requests.filter(r => r._id !== id));
    } catch (err) {
      toast.error(err.response?.data?.message || err.message || 'Error accepting coverage');
    }
  };
  ...
  const decline = async (id) => {
    try {
      ...
      toast.error(err.response?.data?.message || err.message || 'Error declining coverage');
    }
  };
    ...
    <div style={{ maxWidth: 700, margin: '40px auto', padding: 20 }}>
      ...
      {requests.map(r => (
        <div key={r._id} style={{ border: '1px solid #ddd', borderRadius: 10, padding: 16, marginBottom: 16 }}>
          ...
          <p><strong>Period {r.periodNumber || r.period?.periodNumber}:</strong> {r.startTime || r.period?.startTime} - {r.endTime || r.period?.endTime}</p>
          <p><strong>Subject:</strong> {r.subject || r.period?.subject} | <strong>Class:</strong> {r.className || r.period?.className}</p>
          <div style={{ display: 'flex', gap: 10, marginTop: 10 }}>
            <button onClick={() => accept(r._id)}
              style={{ padding: '8px 20px', background: '#059669', color: '#fff', border: 'none', borderRadius: 6, cursor: 'pointer' }}>
              Accept
            </button>
