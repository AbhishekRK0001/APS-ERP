...
async function createCoverage(teacher, body) {
  ...
  const leaveType = (body.leaveType && TYPES.includes(body.leaveType)) ? body.leaveType : 'casual';
  return transaction(async session => {
    ...
    if (existing) {
      if (existing.startDate.getTime() === start.getTime() && existing.endDate.getTime() === end.getTime()) {
    ...
    const requests = await SubstituteRequest.create(periods.map(p => ({ ...p, leave: leave._id })), { session: session || undefined, ordered: true });
...
async function accept(requestId, teacher) {
  return transaction(async session => {
    ...
    if (!claimed) fail('This substitute request has already been accepted by another teacher.', 409);
...
async function submit(leaveId, teacher, body) {
  return transaction(async session => {
    ...
    const leaveType = (body.leaveType && TYPES.includes(body.leaveType)) ? body.leaveType : leave.leaveType;
    ...
    const updated = await Leave.findOneAndUpdate({ _id: leaveId, teacher, status: 'substitute_confirmed' }, { $set: { reason: body.reason.trim(), leaveType, status: 'submitted', submittedAt: new Date() } }, { new: true, session });
...
async function approve(leaveId, role) {
  return transaction(async session => {
    ...
    for (const [k, count] of charges) {
      ...
      await User.updateOne({ _id: leave.teacher }, { $inc: { [field]: count } }, { session });
    }
