...
exports.getMyLeaves = async (req, res) => {
  try { res.json(await Leave.find({ teacher: req.user._id }).populate('substituteTeacher', 'name email').populate({ path: 'substituteRequests', populate: { path: 'substituteTeacher', select: 'name email' } }).sort({ createdAt: -1 })); }
  catch (e) { send(res, e); }
};
...
exports.getAllLeaves = async (req, res) => {
  try { res.json(await Leave.find().populate('teacher substituteTeacher', 'name email').populate({ path: 'substituteRequests', populate: { path: 'substituteTeacher', select: 'name email' } }).sort({ createdAt: -1 })); }
  catch (e) { send(res, e); }
};
