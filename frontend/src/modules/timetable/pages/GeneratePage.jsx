...
export default function GeneratePage() {
  ...
  const subjects = useMemo(() => {
    ...
    return allSubjects.filter(
      (subject) =>
        String(subject.sectionId?._id || subject.sectionId) ===
          String(sectionId) &&
      ...
      .catch((err) => {
        ...
        alert(
          err.response?.data?.message ||
            ...
            "Failed to load saved timetable",
        );
      })
      .finally(() => {
    ...
    <div className="page-shell content-narrow">
      ...
      <div className="card">
        <div className="card-header">
          ...
          <p className="muted">
            Select the class and working period before generating.
          </p>
        </div>
        ...
        <div className="form-grid generate-controls">
          ...
          <label className="field-label">
            ...
            <input
              ...
              onChange={(event) =>
                updateWorkingDate(setStartDate, event.target.value)
              }
            />
          </label>
          ...
          <label className="field-label">
            ...
            <input
              ...
              disabled={!sectionId || validatingMove}
              onChange={(event) =>
                updateWorkingDate(setEndDate, event.target.value)
              }
            />
          </label>
        ...
        {semester && sectionId && subjects.length === 0 && (
          <div className="empty-state">
            No subjects found for this semester and section.
          </div>
        )}
      </div>
        ...
        timetable?.grid?.length > 0 && (
          <>
            <p className="muted edit-instructions">
              Edit manually by clicking a class and then its destination, or
              drag and drop it. Every proposed move is checked before it is
              applied.
            </p>
