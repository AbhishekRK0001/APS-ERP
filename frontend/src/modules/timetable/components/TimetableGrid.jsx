...
function moveOrSwapBlock(data, sourcePosition, targetPosition) {
  ...
  if (targetSlots.some((slot) => !TEACHING_SLOTS.has(slot))) {
    return {
      error: "A class block cannot cross the break, lunch, or end of day.",
    };
  }
  ...
  if (occupiedTargets.length > 0) {
    ...
    if (!target || target.length !== source.length) {
      return {
        error:
          "Classes can only be swapped with another block of the same length.",
      };
    }
...
export default function TimetableGrid({
  ...
  const selectOrMoveClass = async (day, slot, cell) => {
    ...
    if (!selectedSource) {
      if (!cell) {
        onInvalidMove?.(
          "Select an occupied class first, then select its destination.",
        );
        return;
      }
  ...
  return (
    <div
      className="stack"
      style={{ background: "#ffffff", color: "#000000", padding: 12 }}
    >
                ...
                <tr key={dIndex}>
                  ...
                  {SLOT_LABELS.map((_, sIndex) => {
                    ...
                    if (sIndex === 2 && dIndex === 0) {
                      return (
                        <td key={sIndex} rowSpan={6} style={breakStyle}>
                    ...
                    if (sIndex === 5 && dIndex === 0) {
                      return (
                        <td key={sIndex} rowSpan={6} style={lunchStyle}>
                    ...
                    return (
                      <td
                        ...
                        onDragStart={(event) =>
                          startDragging(event, dIndex, sIndex)
                        }
                        onDragEnd={() => {
                          setDragSource(null);
                          setDropTarget(null);
    ...
    day.forEach((cell) => {
      getEntries(cell).forEach((entry) => {
        ...
        if (entry.teacherName)
          map.get(subjectId).teachers.add(entry.teacherName);
      });
    });
  });
